...
bool FrontierNaive::Check(basic::CostVector g) {
  for (auto l_prime: labels) {
    ...
    if (g[0] >= l_prime.second.g[0] && g[1] <= l_prime.second.g[1] && g[2] <= l_prime.second.g[2]) 
    {
        // prned++;
        return true;
    }
...
void FrontierNaive::Update(Label l) {
  ...
  for (auto l_prime: labels) 
    ...
    if (l.g[0] >= l_prime.second.g[0] && l.g[1] <= l_prime.second.g[1] && l.g[2] <= l_prime.second.g[2])
      {
...
int AstarRefill::Search(long vo, long vd, double time_limit) {
  ...
  _res.rt_initHeu = std::chrono::duration<double>(t2-t1).count();
    ...
    std::cout<< "Check before entering OPEN while loop--1"<< std::endl; //--> This is working
  ...
  while ( !_open.empty() ) {
    ...
    std::cout<<"g , q, k , t, cap: "<<l.g[0]<<" "<<l.g[1]<<" "<<l.g[2]<<" "<<l.g[3]<<std::endl;
    ...
    // std::cout<<"Heuristic now is :"<< _Heuristic(l.v)[0] <<std::endl;
    if (_checkForPrune(l)) {
        continue;
    }
    ...
    //   std::cout<< "the neighbours are : "<< u<<" with edge costs "<<_roadmap->GetCost(u, l.v)[0]<<" ";
    //   std::cout <<" and "<<_roadmap->GetCost(u, l.v)[1] << " ";
    ...
    // long g_prime, q_prime;
      ...
      if(_roadmap->GetCost(u, l.v)[0] > c[0])
      {
        ...
        k_prime = l.g[2] + 1;
        // t_prime = l.g[3] + _t_fuel*(_q_max - l.g[1]) + _t_fuel*(_roadmap->GetCost(l.v, u)[1]);
      }
      ...
      else 
      {
        if(_roadmap->GetCost(u, l.v)[1] > l.g[1])
        {
          ...
          q_prime = 0;
          k_prime = l.g[2] + 1;
          // t_prime = l.g[3] + _t_fuel*(_roadmap->GetCost(u, l.v)[1] - l.g[1]) + _t_fuel*(_roadmap->GetCost(u, l.v)[1]);
  ...
  std::cout << "Elapsed time in microseconds: "
    ...
    // std::cout<< "Number of iterations or the number of expanded labels :"<<alg_iter<< std::endl; 
    // std::cout<< "Number of labels pruned : "<< prned << std::endl;
    // long _m = -1;
    ...
    //     // std::cout<<" }"<<" (Min, max) :"<<*std::min_element(i.second.begin(), i.second.end()) <<
    //     //     " "<< *std::max_element(i.second.begin(), i.second.end())<<std::endl;
    //     
    // }
    ...
    // std::cout<< "the size of the entire frontier set for this case is :"<< total_frontier_size<< std::endl;
    ...
    //      which returns a CostVector of length one, and c[0] is the cost value of the edge.
  {
    ...
    while (!OPENv.empty()) 
    {
      ...
      for (auto u_prime : succs) 
      {
        ...
        // std::cout<<"u_prime d: "<< d_star[u_prime]<< " & u d:"<<d_star[u]<<" + edge cost : "<<edge_cost<<std::endl; //--> working as needed. 
